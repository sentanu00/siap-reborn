           ...
           <ul class="breadcrumb-title">
             ...
             <li class="breadcrumb-item">&nbsp;&nbsp;
               <?php if ($this->access['is_add'] == 1) : ?>
                 <a href="javascript:getform('<?php echo site_url('riwayatpendidikan/add') ?>','<?= $PEGAWAI_ID; ?>')" class="btn btn-success btn-round" style="color:white" title="Add New Data">
     ...
     <div class="modal-dialog modal-dialog-centered">
       ...
       <div class="modal-content">
         <div class="modal-header" style="background-color: #2196F3; color: white;">
           ...
           <h4 class="modal-title" style="text-align: center; margin: 0 auto;"><i class="fa fa-globe"></i> Data BKN - Riwayat Pendidikan</h4>
         </div>
   ...
   <script>
     $(document).ready(function() {
       $.ajax({
         url: "<?php echo site_url('webservice_bkn/wsbkn_rw?jenis_api=pendidikan&pegawaiid=') ?><?php echo $PEGAWAI_ID ?>",
         dataType: 'json',
         success: function(response) {
           if (response.code === 1 && response.data.length > 0) {
             ...
             for (var i = 0; i < data.length; i++) {
               ...
               var row = '<tr>' +
                 '<td>' + (i + 1) + '</td>' +
                 '<td>' + rowData.tkPendidikanNama + '</td>' +
                 '<td>' + rowData.pendidikanNama + '</td>' +
                 '<td>' + rowData.tglLulus + '</td>' +
                 '<td>' + rowData.namaSekolah + '</td>' +
                 '<td>' + rowData.nomorIjasah + '</td>' +
                 '</tr>';
   ...
   <script>
     ...
     $(function() {
       ...
       table = $('#gridv').DataTable({
         ...
         "ajax": {
           "url": "<?php echo site_url('riwayatpendidikan/grids') ?>/<?= $PEGAWAI_ID; ?>",
           "type": "POST"
         },
       ...
       $('#gridv').on('dblclick', 'tr', function() {
         ...
         getform('<?php echo site_url('riwayatpendidikan/add') ?>/' + id, '<?= $PEGAWAI_ID; ?>')
         //window.location = "<?= site_url('riwayatpendidikan/add') ?>/"+id;
       });
     });
   </script>