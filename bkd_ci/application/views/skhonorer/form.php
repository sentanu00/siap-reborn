<div class="row">
	<div class="col-md-12">
		...
		<form action="<?php echo site_url('skhonorer/save/' . $row['JABATAN_RIWAYAT_ID']); ?>" class='form-vertical' parsley-validate='true' novalidate='true' method="post" enctype="multipart/form-data">
			...
			<div class="row">
				<div class="col-md-4">
					...
					<div class="form-group" id="jabatan_id">
						...
						<select name="JABATAN_FUNGSIONAL_ID" rows="5" id="JABATAN_FUNGSIONAL_ID" class="form-control input-sm select2" style="width: 100%;" required></select>
					...
					<div class="form-group" id="jft_id_sapk">
						...
						<select name='JFT_ID_SAPK' rows='5' id='JFT_ID_SAPK' required code='{$JFT_ID_SAPK}' class='form-control input-sm select2 ' style='width: 100%;'></select>
					</div>
					<div class="form-group hidethis " style="display:none;">
					...
					<div class="form-group" id="jfu_id_sapk">
						...
						<select name='JFU_ID_SAPK' rows='5' id='JFU_ID_SAPK' code='{$JFU_ID_SAPK}' class='form-control input-sm select2' style='width: 100%;'></select>
					</div>
					<div class="form-group hidethis " style="display:none;">
					...
					<div class="form-group  ">
						...
						<?
						if ($row['FILE_PDF'] != '') {
							echo '<br /><a href="javascript:SximoModal(\'' . site_url('skhonorer/viewfile') . '/FILE_PDF/' . $row['JABATAN_RIWAYAT_ID'] . '\',\'View File\',1000)" class="btn btn-danger"><img src="' . base_url('/assets/icon/adadoc.png') . '" style="width:20px"> Preview File</a>';
						}
						?>
					...
					<div class="form-group  ">
						...
						if ($row['FILE_PELANTIKAN'] != '') {
							echo '<br /><a href="javascript:SximoModal(\'' . site_url('skhonorer/viewfile') . '/FILE_PELANTIKAN/' . $row['JABATAN_RIWAYAT_ID'] . '\',\'View File\',1000)" class="btn btn-danger"><img src="' . base_url('/assets/icon/adadoc.png') . '" style="width:20px"> Preview File</a>';
						}
						?>
					</div>
				</div>
...
<script type="text/javascript">
	...
	function getkelasjabatan() {
		$.ajax({
			type: "GET",
			url: "<?= site_url('skhonorer/getkelas'); ?>/" + $("#JABATAN_FUNGSIONAL_ID").val(),
			dataType: "html",
	...
	$(document).ready(function() {
		...
		jenis_jabatan.addEventListener('change', function() {
			...
			if (data_value === '1') {
				...
				jfuIdElement.disabled = true;
				jfuIdElement.removeAttribute('required');
				$("#JABATAN_FUNGSIONAL_ID").jCombo("<?php echo site_url('skhonorer/comboselect?filter=master_jabatan:id:nm_jabatan|ket') ?>", {
					selected_value: '<?php echo $row["JABATAN_FUNGSIONAL_ID"] ?>'
				});
				// $("#JFT_ID_SAPK").jCombo("<?php echo site_url('skhonorer/comboselect?filter=master_jabatan_fungsional_tertentu:JFT_SIASN_ID:NAMA|BUP_USIA') ?>",{  selected_value : '<?php echo $row["JFT_ID_SAPK"] ?>' });
				$.ajax({
				...
				// $("#JFU_ID_SAPK").jCombo("<?php echo site_url('skhonorer/comboselect?filter=master_jabatan_fungsional_umum:JFU_SIASN_ID:NAMA') ?>",{  selected_value : '<?php echo $row["JFU_ID_SAPK"] ?>' });
			...
			$.ajax({
				...
				success: function(data) {
					...
					if (jabatanRiwayatId != "") {
						$.ajax({
							type: 'GET',
							// url: 'http://localhost:8082/index.php/skhonorer/postSiasn/'+jabatanRiwayatId, 
							url: url + '/skhonorer/postSiasn/' + jabatanRiwayatId, // Replace 'controller' with the actual URL to your controller
							success: function(response) {
		...
		$("#PEJABAT_PENETAP_ID").jCombo("<?php echo site_url('skhonorer/comboselect?filter=pejabat_penetap:PEJABAT_PENETAP_ID:JABATAN') ?>", {
			selected_value: '<?php echo $row["PEJABAT_PENETAP_ID"] ?>'
		});

		$("#JENIS_JABATAN_SAPK").jCombo("<?php echo site_url('skhonorer/comboselect?filter=jenis_jabatan:ID:NAMA') ?>", {
			selected_value: '<?php echo $row["JENIS_JABATAN_SAPK"] ?>'
		});

		$("#SATKER_ID").jCombo("<?php echo site_url('skhonorer/comboselect?filter=satker:SATKER_ID:NAMA') ?>", {
			selected_value: '<?php echo $row["SATKER_ID"] ?>'
		});

		$("#ESELON_ID").jCombo("<?php echo site_url('skhonorer/comboselect?filter=eselon:ESELON_ID:NAMA') ?>", {
	...
	function validateForm() {
		...
		if (data_value === '2') {
			var errorMessage = isEmptyInputPejabat && isEmptyNoSK && isEmptyJenisJabatan && isEmptySatker && isEmptyJabfung && isEmptyTMTJabatan && isEmptyJFT && isEmptyFile ?
				"Mohon kolom Pejabat Penetap, No SK, Jenis Jabatan SAPK, Satuan Kerja, Jabatan, TMT Jabatan, JFT ID SAPK, dan Dokumen SK Jabatan harap diisi." :
				isEmptyInputPejabat ?
				...
				isEmptyJFT ?
				"Mohon JFT ID SAPK harap diisi" :
				isEmptyFile ?
				"Mohon Dokumen SK KP harap diisi" :
				"";
		} else if (data_value === '3') {
			var errorMessage = isEmptyInputPejabat && isEmptyNoSK && isEmptyJenisJabatan && isEmptySatker && isEmptyJabfung && isEmptyTMTJabatan && isEmptyJFU && isEmptyFile ?
				"Mohon kolom Pejabat Penetap, No SK, Jenis Jabatan SAPK, Satuan Kerja, Jabatan, TMT Jabatan, JFU ID SAPK, dan Dokumen SK Jabatan harap diisi." :
				isEmptyInputPejabat ?
				"Mohon kolom Pejabat Penetap diisi." :
				isEmptyNoSK ?
				"Mohon kolom No SK diisi." :
