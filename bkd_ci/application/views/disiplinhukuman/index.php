...
<script>
  ...
  $(function() {
    ...
    table = $('#gridv').DataTable({
      ...
      "ajax": {
        "url": "<?php echo site_url('disiplinhukuman/grids') ?>?satker=0&sttpeg=1,2,10&thn=<?php echo date('Y'); ?>",
        "type": "POST"
      },
    //   "ajax": {
    //     "url": "<?php echo site_url('disiplinhukuman/grids') ?>?satker=0&sttpeg=1,2,10",
    //     "type": "POST"
    //   },
      // Load data for the table's content from an Ajax source
      // "ajax": {
      //   "url": "<?php echo site_url('disiplinhukuman/grids') ?>?satker=0&sttpeg=1,2,10&bln=<?php echo date('m'); ?>",
      //   "type": "POST"
      // },
  ...
  $('#data').on('changed.jstree', function(e, data) {
    ...
    'core': {
      'data': {
        "url": "<?= site_url('disiplinhukuman/satker'); ?>",
        'data': function(node) {
          return {
            'id': node.id
          };
  ...
  function reloadgridx(idsatker = '') {
    ...
    thn = $('#thn').val();
    table.ajax.url("<?php echo site_url('disiplinhukuman/grids') ?>?satker=" + idsatker + "&sttpeg=" + sttpeg + "&thn=" + thn).load();
    // table.ajax.url("<?php echo site_url('disiplinhukuman/grids') ?>?satker=" + idsatker + "&sttpeg=" + sttpeg).load();
  }
  ...
  $("#gridv_length").append('<select class="form-control" id="status_peg" onchange="reloadgridx()" style="width:150px">'+
    '<option value="0,1,2,10,3,4,5,6,7,8,9">SEMUA</option>'+
    ...
    var thnop = '';
    for (var ix = (thnskg - 5); ix <= thnskg; ix++) {
