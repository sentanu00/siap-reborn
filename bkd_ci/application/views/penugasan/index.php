           ...
           <ul class="breadcrumb-title">
             ...
             <li class="breadcrumb-item">&nbsp;&nbsp;
               <?php if ($this->access['is_add'] == 1) : ?>
                 <a href="javascript:getform('<?php echo site_url('penugasan/add') ?>','<?= $PEGAWAI_ID; ?>')" class="btn btn-success btn-round" style="color:white" title="Add New Data">
   ...
   <script>
     ...
     $(function() {
       ...
       table = $('#gridv').DataTable({
         ...
         "ajax": {
           "url": "<?php echo site_url('penugasan/grids') ?>/<?= $PEGAWAI_ID; ?>",
           "type": "POST"
         },
       ...
       $('#gridv').on('dblclick', 'tr', function() {
         ...
         getform('<?php echo site_url('penugasan/add') ?>/' + id, '<?= $PEGAWAI_ID; ?>')
         //window.location = "<?= site_url('penugasan/add') ?>/"+id;
       });
     });
   </script>