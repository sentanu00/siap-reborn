...
<script>
  ...
  $(function() {
    ...
    table = $('#gridv').DataTable({
      ...
      "ajax": {
        "url": "<?php echo site_url('disiplin/grids') ?>?satker=0&sttpeg=1,2,10&thnpen=<?php echo date('Y'); ?>",
        "type": "POST"
      },
  ...
  function reloadgridx(idsatker = '') {
    ...
    thnpen = $('#tahun').val();
    table.ajax.url("<?php echo site_url('disiplin/grids') ?>?satker=" + idsatker + "&sttpeg=" + sttpeg + "&thnpen=" + thnpen).load();
  }
  ...
  $(document).ready(function() {
    ...
    var thnop = '';
    for (var ix = thnskg - 4; ix <= thnskg; ix++) {
