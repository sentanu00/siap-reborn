...
<?php if($this->access['is_add'] ==1) : ?>
    <a href="javascript:getform('<?php echo site_url('workshop/add') ?>','<?=$PEGAWAI_ID;?>')" class="btn btn-success btn-round" style="color:white"  title="Add New Data">
...
var table;
 $(function () {
       // $("#gridv").DataTable();
        ...
        "ajax": {
            "url": "<?php echo site_url('workshop/grids')?>/<?=$PEGAWAI_ID;?>",
            "type": "POST"
        },
        ...
        getform('<?php echo site_url('workshop/add') ?>/'+id,'<?=$PEGAWAI_ID;?>')
        //window.location = "<?=site_url('workshop/add')?>/"+id;
    });
      });
</script>
