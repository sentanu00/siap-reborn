...
<script>
  ...
  $(function() {
    ...
    table = $('#gridv').DataTable({
      ...
      "ajax": {
        "url": "<?php echo site_url('disiplincuti/grids') ?>?satker=0&sttpeg=1,2,10&thn=<?php echo date('Y'); ?>",
        "type": "POST"
      },
    //   "ajax": {
    //     "url": "<?php echo site_url('disiplincuti/grids') ?>?satker=0&sttpeg=1,2,10",
    //     "type": "POST"
    //   },
      // Load data for the table's content from an Ajax source
      // "ajax": {
      //   "url": "<?php echo site_url('disiplincuti/grids') ?>?satker=0&sttpeg=1,2,10&bln=<?php echo date('m'); ?>",
      //   "type": "POST"
      // },
  ...
  $('#data').on('changed.jstree', function(e, data) {
    ...
    'core': {
      'data': {
        "url": "<?= site_url('disiplincuti/satker'); ?>",
        'data': function(node) {
          return {
            'id': node.id
          };
  ...
  function reloadgridx(idsatker = '') {
    ...
    thn = $('#thn').val();
    table.ajax.url("<?php echo site_url('disiplincuti/grids') ?>?satker=" + idsatker + "&sttpeg=" + sttpeg + "&thn=" + thn).load();
    // table.ajax.url("<?php echo site_url('disiplincuti/grids') ?>?satker=" + idsatker + "&sttpeg=" + sttpeg).load();
  }
