...
class Nikah extends SB_Controller
{
	...
	where t.group_id = 1 and p.VALIDASI = 0');
		...
		foreach ($rows as $dt) {
			...
			if ($this->access['is_remove'] == 1) {
				...
				$btn .= '<a class="dropdown-item waves-effect waves-light" href="#" onclick="ConfirmDelete(\'' . site_url('nikah/destroy/') . '\',' . $dt->$idku . ')"><i class="ti-trash"></i> Delete</a>';
			}
			$btn .= '</div>';
			if ($dt->FILE_PDF != '') {
				$row[] = '<a href="javascript:SximoModal(\'' . site_url('nikah/viewfile/FILE_PDF') . '/' . $dt->$idku . '\',\'View File\',1000)"><img src="' . base_url('/assets/icon/adadoc.png') . '" style="width:20px"></a>';
			} else {
	...
	// {
	// 	$sq = $this->db->query("SELECT FILE_PDF FROM nikah_riwayat WHERE NIKAH_RIWAYAT_ID = '$id'")->row();
	...
	// 	*/
	// 	echo '<iframe src="'.base_url('temp.pdf').'" width="100%" height="600px"></iframe>';
	// }
	...
	function index()
	{
		...
		echo $this->data['content'] = $this->load->view('nikah/index', $this->data, true);
	...
	function show($id = null)
	{
		...
		if ($row) {
			...
			$this->data['row'] = $this->model->getColumnTable('nikah_riwayat');
		}

		$this->data['id'] = $id;
		echo $this->data['content'] =  $this->load->view('nikah/view', $this->data, true);
		//$this->load->view('layouts/main',$this->data);
	}
	...
	function add($id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['PEGAWAI_ID'] = $_POST['id'];
		echo $this->data['content'] = $this->load->view('nikah/form', $this->data, true);
		//$this->load->view('layouts/main', $this->data );
	...
	{
		$th = $this->db->query("SELECT $col FROM nikah_riwayat WHERE NIKAH_RIWAYAT_ID = '$id'")->row();
		$ext = explode(".", $th->$col);
		$maxext = count($ext);
		$extn = $ext[$maxext - 1];
		if ($extn == 'pdf') {
	...
	function save()
	{
		...
		// 	$ID = $this->model->insertRow($data, $this->input->get_post('NIKAH_RIWAYAT_ID', true));
		// 	// Input logs
		// 	if ($this->input->get('NIKAH_RIWAYAT_ID', true) == '') {
		...
		if ($tempfile_sk == '') {
			...
			$_FILES["FILE_PDF"]["name"] = 'Nikah_' . $nip . '_' . $tmt . '.pdf';
			if (!$this->upload->do_upload('FILE_PDF')) {
				$e = $this->upload->display_errors();
				$a = $e;
			} else {
				$namafile_sk = 'dokumen/' . $nip . '/Nikah_' . $nip . '_' . $tmt . '.pdf';
			}
		}
		...
		if ($this->form_validation->run()) {
			...
			$datalama = $this->getdatalama($_POST, 'nikah_riwayat', 'NIKAH_RIWAYAT_ID', $data['NIKAH_RIWAYAT_ID']);
			$ID = $this->model->insertRow($data, $this->input->get_post('NIKAH_RIWAYAT_ID', true));
			$this->perubahandata($data['PEGAWAI_ID'], 'nikah', 'nikah_riwayat', json_encode($datalama), json_encode($_POST), 'NIKAH_RIWAYAT_ID', $ID);
			// Input logs
			if ($this->input->get('NIKAH_RIWAYAT_ID', true) == '') {
				$this->inputLogs("New Entry row with ID : $ID  , Has Been Save Successfull");
			} else {
