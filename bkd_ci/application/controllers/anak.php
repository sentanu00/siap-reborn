...
class Anak extends SB_Controller
{
	...
	function __construct()
	{
		...
		$this->load->model('anakmodel');
		$this->model = $this->anakmodel;
	...
	where t.group_id = 1 and p.VALIDASI = 0');
		...
		foreach ($rows as $dt) {
			...
			if ($this->access['is_remove'] == 1) {
				$btn .= '<a class="dropdown-item waves-effect waves-light" href="#" onclick="ConfirmDelete(\'' . site_url('anak/destroy/') . '\',' . $dt->$idku . ')"><i class="ti-trash"></i> Delete</a>';
			}
			$btn .= '</div>';
			if ($dt->FILE_PDF != '') {
				$row[] = '<a href="javascript:SximoModal(\'' . site_url('anak/viewfile/FILE_PDF') . '/' . $dt->$idku . '\',\'View File\',1000)"><img src="' . base_url('/assets/icon/adadoc.png') . '" style="width:20px"></a>';
			} else {
				$row[] = '<img src="' . base_url('/assets/icon/nodoc.png') . '" style="width:20px">';
			}
			//	$row[] = '<a href="javascript:SximoModal(\'' . site_url('dfsview/anak') . '/' . $pg . '/' . $dt->$idku . '\',\'Data DFS\',1000)">View File</a>';
			$row[] = $btn;
			$data[] = $row;
			$no++;
		}
	...
	function index()
	{
		...
		echo $this->data['content'] = $this->load->view('anak/index', $this->data, true);
	...
	function show($id = null)
	{
		...
		if ($row) {
			...
			$this->data['row'] = $this->model->getColumnTable('anak');
		}

		$this->data['id'] = $id;
		echo $this->data['content'] =  $this->load->view('anak/view', $this->data, true);
		//$this->load->view('layouts/main',$this->data);
	}
	...
	function add($id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['PEGAWAI_ID'] = $_POST['id'];
		echo $this->data['content'] = $this->load->view('anak/form', $this->data, true);
		//$this->load->view('layouts/main', $this->data );
	...
	{
		$th = $this->db->query("SELECT $col FROM anak WHERE ANAK_ID = '$id'")->row();
		$ext = explode(".", $th->$col);
		$maxext = count($ext);
		$extn = $ext[$maxext - 1];
		if ($extn == 'pdf') {
	...
	function save()
	{
		...
		*/
		...
		if ($tempfile_sk == '') {
			...
			$_FILES["FILE_PDF"]["name"] = 'AKTA_ANAK_' . $nip . '_' . $tmt . '.pdf';
			if (!$this->upload->do_upload('FILE_PDF')) {
				$e = $this->upload->display_errors();
				$a = $e;
			} else {
				$namafile_sk = 'dokumen/' . $nip . '/AKTA_ANAK_' . $nip . '_' . $tmt . '.pdf';
			}
		}
		...
		if ($this->form_validation->run()) {
			...
			$datalama = $this->getdatalama($_POST, 'anak', 'ANAK_ID', $data['ANAK_ID']);
			$ID = $this->model->insertRow($data, $this->input->get_post('ANAK_ID', true));
			$this->perubahandata($data['PEGAWAI_ID'], 'Anak', 'anak', json_encode($datalama), json_encode($_POST), 'ANAK_ID', $ID);
			// Input logs
			if ($this->input->get('ANAK_ID', true) == '') {
	...
	function destroy()
	{
		...
		$datalama = $this->getdatalamadelete('anak', 'ANAK_ID', $_POST['id']);
		$this->perubahandata($datalama['PEGAWAI_ID'], 'Anak', 'anak', json_encode($datalama), 'DELETE', 'ANAK_ID', $_POST['id']);
