...
class Riwayatpendidikan extends SB_Controller
{

	protected $layout 	= "layouts/main";
	public $module 		= 'riwayatpendidikan';
	public $per_page	= '10';
	public $idx			= '';
	...
	function __construct()
	{
		...
		$this->load->model('riwayatpendidikanmodel');
		$this->model = $this->riwayatpendidikanmodel;
...
<div class="dropdown-menu" x-placement="bottom-start" style="position: absolute; transform: translate3d(86px, 40px, 0px); top: 0px; left: 0px; will-change: transform;">';
			...
			if ($this->access['is_remove'] == 1) {
				$btn .= '<a class="dropdown-item waves-effect waves-light" href="#" onclick="ConfirmDelete(\'' . site_url('riwayatpendidikan/destroy/') . '\',' . $dt->$idku . ')"><i class="ti-trash"></i> Delete</a>';
			}
			$btn .= '</div>';
			if ($dt->FILE_PDF != '') {
				$row[] = '<a href="javascript:SximoModal(\'' . site_url('riwayatpendidikan/viewfile/FILE_PDF') . '/' . $dt->$idku . '\',\'View File\',1000)"><img src="' . base_url('/assets/icon/adadoc.png') . '" style="width:20px"></a>';
			} else {
				$row[] = '<img src="' . base_url('/assets/icon/nodoc.png') . '" style="width:20px">';
			}
			//$row[] = '<a href="javascript:SximoModal(\'' . site_url('dfsview/riwayatpendidikan') . '/' . $pg . '/' . $dt->$idku . '\',\'Data DFS\',1000)">View File</a>';
			$row[] = $btn;
			$data[] = $row;
			$no++;
		}
	...
	function index()
	{
		...
		echo $this->data['content'] = $this->load->view('riwayatpendidikan/index', $this->data, true);
	...
	function show($id = null)
	{
		...
		if ($row) {
			...
			$this->data['row'] = $this->model->getColumnTable('pendidikan_riwayat');
		}

		$this->data['id'] = $id;
		echo $this->data['content'] =  $this->load->view('riwayatpendidikan/view', $this->data, true);
		//$this->load->view('layouts/main',$this->data);
	}
	...
	function add($id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['PEGAWAI_ID'] = $_POST['id'];
		echo $this->data['content'] = $this->load->view('riwayatpendidikan/form', $this->data, true);
		//$this->load->view('layouts/main', $this->data );
	...
	{
		$th = $this->db->query("SELECT $col FROM pendidikan_riwayat WHERE PENDIDIKAN_RIWAYAT_ID = '$id'")->row();
		$ext = explode(".", $th->$col);
		$maxext = count($ext);
		$extn = $ext[$maxext - 1];
		if ($extn == 'pdf') {
	...
	function save()
	{
		...
		if ($tempfile == '') {
			...
			$formatberkas = "IJAZAH";
			$tmt = $this->input->post('TANGGAL_STTB');
		...
		*/
		...
		$namafile_transkrip = '';
		$namafile_skgelar = '';
		$tempfile_ijazah = $_FILES['FILE_PDF']['tmp_name'];
		$tempfile_transkrip = $_FILES['FILE_TRANSKRIP']['tmp_name'];
		$tempfile_skgelar = $_FILES['FILE_SK_GELAR']['tmp_name'];
		$tmt = $_POST['TANGGAL_STTB'];
		...
		//IJAZAH
		if ($tempfile_ijazah == '') {
			$namafile_ijazah = $_POST['file_pdf_cek'];
		} else {
			if ($namafile_ijazah != '') unlink($_POST['file_pdf_cek']);
			$_FILES["FILE_PDF"]["name"] = 'PENDIDIKAN_IJZ_' . $nip . '_' . $tmt . '.pdf';
			if (!$this->upload->do_upload('FILE_PDF')) {
				$e = $this->upload->display_errors();
				$a = $e;
			} else {
				$namafile_ijazah = 'dokumen/' . $nip . '/PENDIDIKAN_IJZ_' . $nip . '_' . $tmt . '.pdf';
			}
		}


		//TRANSKRIP
		if ($tempfile_transkrip == '') {
			$namafile_transkrip = $_POST['file_transkrip_cek'];
		} else {
			if ($namafile_transkrip != '') unlink($_POST['file_transkrip_cek']);
			$_FILES["FILE_TRANSKRIP"]["name"] = 'PENDIDIKAN_TRP_' . $nip . '_' . $tmt . '.pdf';
			if (!$this->upload->do_upload('FILE_TRANSKRIP')) {
				$e = $this->upload->display_errors();
				$a = $e;
			} else {
				$namafile_transkrip = 'dokumen/' . $nip . '/PENDIDIKAN_TRP_' . $nip . '_' . $tmt . '.pdf';
			}
		}

		//SK GELAR
		if ($tempfile_skgelar == '') {
			$namafile_skgelar = $_POST['file_skgelar_cek'];
		} else {
			if ($namafile_skgelar != '') unlink($_POST['file_skgelar_cek']);
			$_FILES["FILE_SK_GELAR"]["name"] = 'PENDIDIKAN_SKG_' . $nip . '_' . $tmt . '.pdf';
			if (!$this->upload->do_upload('FILE_SK_GELAR')) {
				$e = $this->upload->display_errors();
				$a = $e;
			} else {
				$namafile_skgelar = 'dokumen/' . $nip . '/PENDIDIKAN_SKG_' . $nip . '_' . $tmt . '.pdf';
			}
		}


		unset($_POST['file_pdf_cek']);
		unset($_POST['file_transkrip_cek']);
		unset($_POST['file_skgelar_cek']);
		...
		if ($this->form_validation->run()) {
			...
			$data['FILE_PDF'] = $namafile_ijazah;
			$data['FILE_TRANSKRIP'] = $namafile_transkrip;
			$data['FILE_SK_GELAR'] = $namafile_skgelar;
			$data['LAST_UPDATE_USER'] = @$user_update;
			$data['LAST_UPDATE_DATE'] = date("Y-m-d H:i:s");

			$data['JURUSAN'] = $_POST['JURUSAN'];
			$datalama = $this->getdatalama($_POST, 'pendidikan_riwayat', 'PENDIDIKAN_RIWAYAT_ID', $data['PENDIDIKAN_RIWAYAT_ID']);
			$ID = $this->model->insertRow($data, $this->input->get_post('PENDIDIKAN_RIWAYAT_ID', true));
			$this->perubahandata($data['PEGAWAI_ID'], 'riwayatpendidikan', 'pendidikan_riwayat', json_encode($datalama), json_encode($_POST), 'PENDIDIKAN_RIWAYAT_ID', $ID);

			$this->getlastriwayat('pendidikan_riwayat', $data['PEGAWAI_ID'], 'TANGGAL_STTB');
			// Input logs
			if ($this->input->get('PENDIDIKAN_RIWAYAT_ID', true) == '') {
	...
	function destroy()
	{
		...
		$datalama = $this->getdatalamadelete('pendidikan_riwayat', 'PENDIDIKAN_RIWAYAT_ID', $_POST['id']);
		$this->perubahandata($datalama['PEGAWAI_ID'], 'Riwayat Pendidikan', 'pendidikan_riwayat', json_encode($datalama), 'DELETE', 'PENDIDIKAN_RIWAYAT_ID', $_POST['id']);
