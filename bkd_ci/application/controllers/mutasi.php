...
class Mutasi extends SB_Controller
{
	...
	function __construct()
	{
		...
		$this->api_mws_token = $this->mutasimodel->getApiMwsToken();
		...
		$this->sso_token = $this->mutasimodel->getSsoToken();
	...
	function grids($pg)
	{
		...
		foreach ($rows as $dt) {
			...
			// 	$btn2 = '';
			// 	$btn2 .= '<button type="button" class="btn btn-info btn-round" style="color:white;font-size:14px" data-toggle="modal" data-target="#modalupdateFILEPDF_' . $dt->$idku . '"><i class="fa fa-globe"></i> Upload File</button>';

			// 	$btn2 .= '<div class="modal fade" id="modalupdateFILEPDF_' . $dt->$idku . '" tabindex="-1" role="dialog" aria-labelledby="modalupdateFILEPDFLabel" aria-hidden="true">
			// 	<div class="modal-dialog" role="document">
			// 	  <div class="modal-content">
			// 		<div class="modal-header">
			// 		  <h5 class="modal-title" id="modalupdateFILEPDFLabel">Update File SK</h5>
			...
			// 		  <form action="' . site_url('mutasi/save/' . $dt->$idku) . '" parsley-validate="true" novalidate="true" method="post" enctype="multipart/form-data">
			...
			// tambahan	
			if ($dt->FILE_PDF != '') {
				$row[] = '<a href="javascript:SximoModal(\'' . site_url('mutasi/viewfile/FILE_PDF') . '/' . $dt->$idku . '\',\'View File\',1000)"><img src="' . base_url('/assets/icon/adadoc.png') . '" style="width:20px"></a>';
			} else {
	...
	function index()
	{
		...
		echo $this->data['content'] = $this->load->view('mutasi/index', $this->data, true);
	...
	function show($id = null)
	{
		...
		$this->data['id'] = $id;
		echo $this->data['content'] =  $this->load->view('mutasi/view', $this->data, true);
		//$this->load->view('layouts/main',$this->data);
	}
	...
	function add($id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['PEGAWAI_ID'] = $_POST['id'];
		echo $this->data['content'] = $this->load->view('mutasi/form', $this->data, true);
		//$this->load->view('layouts/main', $this->data );
	...
	function addfrontend($id = null)
	{
		...
		// $this->data['content'] = $this->load->view('mutasi/form', $this->data, true);
	}
