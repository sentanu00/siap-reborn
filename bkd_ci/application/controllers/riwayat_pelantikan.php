...
class Riwayat_pelantikan extends SB_Controller
{

	protected $layout 	= "layouts/main";
	public $module 		= 'riwayat_pelantikan';
	public $per_page	= '10';
	public $idx			= '';
	...
	function __construct()
	{
		...
		$this->load->model('riwayat_pelantikanmodel');
		$this->model = $this->riwayat_pelantikanmodel;
...
<div class="dropdown-menu" x-placement="bottom-start" style="position: absolute; transform: translate3d(86px, 40px, 0px); top: 0px; left: 0px; will-change: transform;">';
			...
			if ($this->access['is_remove'] == 1) {
				$btn .= '<a class="dropdown-item waves-effect waves-light" href="#" onclick="ConfirmDelete(\'' . site_url('riwayat_pelantikan/destroy/') . '\',' . $dt->$idku . ')"><i class="ti-trash"></i> Delete</a>';
			}
			if ($dt->jenisMutasiId != '') {
				// $btn .= '<a class="dropdown-item waves-effect waves-light" href="#" onclick="ConfirmKirimSiasn(\'' . site_url('riwayat_pelantikan/kirimsiasn') . '\',' . $dt->$idku . ')"><i></i> Kirim Data Ke SIASN</a>';
				// $btn .= '<a class="dropdown-item waves-effect waves-light" href="#" onclick="ConfirmKirimFileSiasn(\'' . site_url('riwayat_pelantikan/kirimfilesiasn') . '\',' . $dt->$idku . ')"><i></i> Kirim File Ke SIASN</a>';
			}
			$btn .= '</div>';
			if ($dt->FILE_PDF != '') {
				$row[] = '<a href="javascript:SximoModal(\'' . site_url('riwayat_pelantikan/viewfile/FILE_PDF') . '/' . $dt->$idku . '\',\'View File\',1000)"><img src="' . base_url('/assets/icon/adadoc.png') . '" style="width:20px"></a>';
			} else {
	...
	function index()
	{
		...
		echo $this->data['content'] = $this->load->view('riwayat_pelantikan/index', $this->data, true);
	...
	function show($id = null)
	{
		...
		$this->data['id'] = $id;
		echo $this->data['content'] =  $this->load->view('riwayat_pelantikan/view', $this->data, true);
		//$this->load->view('layouts/main',$this->data);
	}
	...
	function add($id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['PEGAWAI_ID'] = $_POST['id'];
		echo $this->data['content'] = $this->load->view('riwayat_pelantikan/form', $this->data, true);
		//$this->load->view('layouts/main', $this->data );
	...
	function save()
	{
		...
		if ($this->form_validation->run()) {
			...
			// 	redirect('riwayat_pelantikan/add/' . $ID, 301);
			// } else {
			// 	redirect('riwayat_pelantikan', 301);
			// }
