...
class Seminar extends SB_Controller
{

	protected $layout 	= "layouts/main";
	public $module 		= 'seminar';
	public $per_page	= '10';
	public $idx			= '';
	...
	where t.group_id = 1 and p.VALIDASI = 0');
		...
		foreach ($rows as $dt) {
			...
			if ($this->access['is_remove'] == 1) {
				$btn .= '<a class="dropdown-item waves-effect waves-light" href="#" onclick="ConfirmDelete(\'' . site_url('seminar/destroy/') . '\',' . $dt->$idku . ')"><i class="ti-trash"></i> Delete</a>';
			}
			$btn .= '</div>';
			if ($dt->FILE_PDF != '') {
				$row[] = '<a href="javascript:SximoModal(\'' . site_url('seminar/viewfile/FILE_PDF') . '/' . $dt->$idku . '\',\'View File\',1000)"><img src="' . base_url('/assets/icon/adadoc.png') . '" style="width:20px"></a>';
			} else {
	...
	function index()
	{
		...
		echo $this->data['content'] = $this->load->view('seminar/index', $this->data, true);
	...
	function show($id = null)
	{
		...
		if ($row) {
			...
			$this->data['row'] = $this->model->getColumnTable('seminar');
		}

		$this->data['id'] = $id;
		echo $this->data['content'] =  $this->load->view('seminar/view', $this->data, true);
		//$this->load->view('layouts/main',$this->data);
	}
	...
	function add($id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['PEGAWAI_ID'] = $_POST['id'];
		echo $this->data['content'] = $this->load->view('seminar/form', $this->data, true);
		//$this->load->view('layouts/main', $this->data );
	...
	{
		$th = $this->db->query("SELECT $col FROM seminar WHERE SEMINAR_ID = '$id'")->row();
		$ext = explode(".", $th->$col);
		$maxext = count($ext);
		$extn = $ext[$maxext - 1];
		if ($extn == 'pdf') {
	...
	function save()
	{
		...
		*/
		...
		if ($tempfile_sk == '') {
			...
			$_FILES["FILE_PDF"]["name"] = 'SEMINAR_' . $nip . '_' . $tmt . '.pdf';
			if (!$this->upload->do_upload('FILE_PDF')) {
				$e = $this->upload->display_errors();
				$a = $e;
			} else {
				$namafile_sk = 'dokumen/' . $nip . '/SEMINAR_' . $nip . '_' . $tmt . '.pdf';
			}
		}
		...
		if ($this->form_validation->run()) {
			...
			$datalama = $this->getdatalama($_POST, 'seminar', 'SEMINAR_ID', $data['SEMINAR_ID']);
			$ID = $this->model->insertRow($data, $this->input->get_post('SEMINAR_ID', true));
			$this->perubahandata($data['PEGAWAI_ID'], 'Seminar', 'seminar', json_encode($datalama), json_encode($_POST), 'SEMINAR_ID', $ID);
			// Input logs
			if ($this->input->get('SEMINAR_ID', true) == '') {
	...
	function destroy()
	{
		...
		$datalama = $this->getdatalamadelete('seminar', 'SEMINAR_ID', $_POST['id']);
		$this->perubahandata($datalama['PEGAWAI_ID'], 'Seminar', 'seminar', json_encode($datalama), 'DELETE', 'SEMINAR_ID', $_POST['id']);
