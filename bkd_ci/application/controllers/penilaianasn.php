...
class Penilaianasn extends SB_Controller
{

	protected $layout 	= "layouts/main";
	public $module 		= 'penilaianasn';
	public $per_page	= '10';
	public $idx			= '';
	...
	function __construct()
	{
		...
		$this->load->model('penilaianasnmodel');
		$this->model = $this->penilaianasnmodel;
	...
	function grids($pg)
	{
		...
		foreach ($rows as $dt) {
			...
			if ($this->access['is_remove'] == 1) {
				$btn .= '<a class="dropdown-item waves-effect waves-light" href="#" onclick="ConfirmDelete(\'' . site_url('penilaianasn/destroy/') . '\',' . $dt->$idku . ')"><i class="ti-trash"></i> Delete</a>';
			}
			$btn .= '</div>';
			if($dt->FILE_PDF != ''){
				$row[] = '<a href="javascript:SximoModal(\'' . site_url('penilaianasn/viewfile/FILE_PDF') . '/' . $dt->$idku . '\',\'View File\',1000)"><img src="'.base_url('/assets/icon/adadoc.png').'" style="width:20px"></a>';
				}else{
					$row[] = '<img src="'.base_url('/assets/icon/nodoc.png').'" style="width:20px">';
				}
		//	$row[] = '<a href="javascript:SximoModal(\'' . site_url('dfsview/penilaianskp') . '/' . $pg . '/' . $dt->$idku . '\',\'Data DFS\',1000)">View File</a>';
			$row[] = $btn;
			$data[] = $row;
			$no++;
		}
	...
	function index()
	{
		...
		echo $this->data['content'] = $this->load->view('penilaianasn/index', $this->data, true);
	...
	function show($id = null)
	{
		...
		if ($row) {
			...
			$this->data['row'] = $this->model->getColumnTable('penilaian_skp');
		}

		$this->data['id'] = $id;
		echo $this->data['content'] =  $this->load->view('penilaianasn/view', $this->data, true);
		//$this->load->view('layouts/main',$this->data);
	}
	...
	function add($id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['PEGAWAI_ID'] = $_POST['id'];
		echo $this->data['content'] = $this->load->view('penilaianasn/form', $this->data, true);
		//$this->load->view('layouts/main', $this->data );

	}
	
	function viewfile($col,$id){
		$th = $this->db->query("SELECT $col FROM penilaian_skp WHERE PENILAIAN_SKP_ID = '$id'")->row();
		$ext = explode(".",$th->$col);
		$maxext = count($ext);
		$extn = $ext[$maxext-1];
		if($extn == 'pdf'){
	...
	function save()
	{
		...
		if ($tempfile == '') {
			...
			$formatberkas = "SKP";
			$tmt = $this->input->post('TAHUN')."-01-01";
		...
		*/
		...
		if ($this->form_validation->run()) {
			...
			$datalama = $this->getdatalama($_POST, 'penilaian_skp', 'PENILAIAN_SKP_ID', $data['PENILAIAN_SKP_ID']);
			$ID = $this->model->insertRow($data, $this->input->get_post('PENILAIAN_SKP_ID', true));
			$this->perubahandata($data['PEGAWAI_ID'], 'penilaianasn', 'penilaian_skp', json_encode($datalama), json_encode($_POST), 'PENILAIAN_SKP_ID', $ID);
			// Input logs
			if ($this->input->get('PENILAIAN_SKP_ID', true) == '') {
