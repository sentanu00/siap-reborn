...
class Organisasi extends SB_Controller
{

	protected $layout 	= "layouts/main";
	public $module 		= 'organisasi';
	public $per_page	= '10';
	public $idx			= '';
	...
	function __construct()
	{
		...
		$this->model = $this->organisasimodel;
	...
	where t.group_id = 1 and p.VALIDASI = 0');
		...
		foreach ($rows as $dt) {
			...
			if ($this->access['is_remove'] == 1) {
				$btn .= '<a class="dropdown-item waves-effect waves-light" href="#" onclick="ConfirmDelete(\'' . site_url('organisasi/destroy/') . '\',' . $dt->$idku . ')"><i class="ti-trash"></i> Delete</a>';
			}
			$btn .= '</div>';
			if ($dt->FILE_PDF != '') {
				$row[] = '<a href="javascript:SximoModal(\'' . site_url('organisasi/viewfile/FILE_PDF') . '/' . $dt->$idku . '\',\'View File\',1000)"><img src="' . base_url('/assets/icon/adadoc.png') . '" style="width:20px"></a>';
			} else {
	...
	function index()
	{
		...
		echo $this->data['content'] = $this->load->view('organisasi/index', $this->data, true);
	...
	function show($id = null)
	{
		...
		if ($row) {
			...
			$this->data['row'] = $this->model->getColumnTable('organisasi_riwayat');
		}

		$this->data['id'] = $id;
		echo $this->data['content'] =  $this->load->view('organisasi/view', $this->data, true);
		//$this->load->view('layouts/main',$this->data);
	}
	...
	function add($id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['PEGAWAI_ID'] = $_POST['id'];
		echo $this->data['content'] = $this->load->view('organisasi/form', $this->data, true);
		//$this->load->view('layouts/main', $this->data );
	...
	{
		$th = $this->db->query("SELECT $col FROM organisasi_riwayat WHERE ORGANISASI_RIWAYAT_ID = '$id'")->row();
		$ext = explode(".", $th->$col);
		$maxext = count($ext);
		$extn = $ext[$maxext - 1];
		if ($extn == 'pdf') {
	...
	function save()
	{
		...
		if ($tempfile == '') {
			...
			$formatberkas = "SK_ORGANISASI";
			$tmt = $this->input->post('TANGGAL_AWAL');
		...
		*/
		...
		if ($tempfile_sk == '') {
			...
			$_FILES["FILE_PDF"]["name"] = 'SK_ORGANISASI_' . $nip . '_' . $tmt . '.pdf';
			if (!$this->upload->do_upload('FILE_PDF')) {
				$e = $this->upload->display_errors();
				$a = $e;
			} else {
				$namafile_sk = 'dokumen/' . $nip . '/SK_ORGANISASI_' . $nip . '_' . $tmt . '.pdf';
			}
		}
		...
		if ($this->form_validation->run()) {
			...
			$datalama = $this->getdatalama($_POST, 'organisasi_riwayat', 'ORGANISASI_RIWAYAT_ID', $data['ORGANISASI_RIWAYAT_ID']);
			$ID = $this->model->insertRow($data, $this->input->get_post('ORGANISASI_RIWAYAT_ID', true));
			$this->perubahandata($data['PEGAWAI_ID'], 'Organisasi', 'organisasi_riwayat', json_encode($datalama), json_encode($_POST), 'ORGANISASI_RIWAYAT_ID', $ID);
			// Input logs
			if ($this->input->get('ORGANISASI_RIWAYAT_ID', true) == '') {
