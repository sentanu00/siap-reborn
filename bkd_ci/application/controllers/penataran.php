...
class Penataran extends SB_Controller
{

	protected $layout 	= "layouts/main";
	public $module 		= 'penataran';
	public $per_page	= '10';
	public $idx			= '';
	...
	function __construct()
	{
		...
		$this->load->model('penataranmodel');
		$this->model = $this->penataranmodel;
	...
	where t.group_id = 1 and p.VALIDASI = 0');
		...
		foreach ($rows as $dt) {
			...
			if ($this->access['is_remove'] == 1) {
				$btn .= '<a class="dropdown-item waves-effect waves-light" href="#" onclick="ConfirmDelete(\'' . site_url('penataran/destroy/') . '\',' . $dt->$idku . ')"><i class="ti-trash"></i> Delete</a>';
			}
			$btn .= '</div>';
			if ($dt->FILE_PDF != '') {
				$row[] = '<a href="javascript:SximoModal(\'' . site_url('penataran/viewfile/FILE_PDF') . '/' . $dt->$idku . '\',\'View File\',1000)"><img src="' . base_url('/assets/icon/adadoc.png') . '" style="width:20px"></a>';
			} else {
	...
	function index()
	{
		...
		echo $this->data['content'] = $this->load->view('penataran/index', $this->data, true);
	...
	function show($id = null)
	{
		...
		if ($row) {
			...
			$this->data['row'] = $this->model->getColumnTable('penataran');
		}

		$this->data['id'] = $id;
		echo $this->data['content'] =  $this->load->view('penataran/view', $this->data, true);
		//$this->load->view('layouts/main',$this->data);
	}
	...
	function add($id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['PEGAWAI_ID'] = $_POST['id'];
		echo $this->data['content'] = $this->load->view('penataran/form', $this->data, true);
		//$this->load->view('layouts/main', $this->data );
	...
	{
		$th = $this->db->query("SELECT $col FROM penataran_seminar WHERE PENATARAN_SEMINAR_ID = '$id'")->row();
		$ext = explode(".", $th->$col);
		$maxext = count($ext);
		$extn = $ext[$maxext - 1];
		if ($extn == 'pdf') {
	...
	function save()
	{
		...
		if ($tempfile == '') {
			...
			$formatberkas = "PIAGAM";
			$tmt = $this->input->post('TANGGAL_PIAGAM');
		...
		*/
		...
		if ($tempfile_sk == '') {
			...
			$_FILES["FILE_PDF"]["name"] = 'PENATARAN_' . $nip . '_' . $tmt . '.pdf';
			if (!$this->upload->do_upload('FILE_PDF')) {
				$e = $this->upload->display_errors();
				$a = $e;
			} else {
				$namafile_sk = 'dokumen/' . $nip . '/PENATARAN_' . $nip . '_' . $tmt . '.pdf';
			}
		}
		...
		if ($this->form_validation->run()) {
			...
			unset($data['PENATARAN_ID']);
			$data['FILE_PDF'] = $namafile_sk;
			$data['LAST_UPDATE_USER'] = @$user_update;
			$data['LAST_UPDATE_DATE'] = date("Y-m-d H:i:s");
			$data['PENATARAN_SEMINAR_ID'] = $_POST['PENATARAN_SEMINAR_ID'];
			$datalama = $this->getdatalama($_POST, 'penataran_seminar', 'PENATARAN_SEMINAR_ID', $data['PENATARAN_SEMINAR_ID']);
			$ID = $this->model->insertRow($data, $this->input->get_post('PENATARAN_SEMINAR_ID', true));
			$this->perubahandata($data['PEGAWAI_ID'], 'Penataran', 'penataran_seminar', json_encode($datalama), json_encode($_POST), 'PENATARAN_SEMINAR_ID', $ID);
			// Input logs
			if ($this->input->get('PENATARAN_SEMINAR_ID', true) == '') {
	...
	function destroy()
	{
		...
		$datalama = $this->getdatalamadelete('penataran', 'PENATARAN_SEMINAR_ID', $_POST['id']);
		$this->perubahandata($datalama['PEGAWAI_ID'], 'Penataran', 'penataran', json_encode($datalama), 'DELETE', 'PENATARAN_SEMINAR_ID', $_POST['id']);
