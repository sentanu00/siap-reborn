...
class Workshop extends SB_Controller
{
	...
	function grids($pg)
	{
		...
		foreach ($rows as $dt) {
			...
			if ($this->access['is_remove'] == 1) {
				$btn .= '<a class="dropdown-item waves-effect waves-light" href="#" onclick="ConfirmDelete(\'' . site_url('workshop/destroy/') . '\',' . $dt->$idku . ')"><i class="ti-trash"></i> Delete</a>';
			}
			$btn .= '</div>';
			if($dt->FILE_PDF != ''){
				$row[] = '<a href="javascript:SximoModal(\'' . site_url('workshop/viewfile/FILE_PDF') . '/' . $dt->$idku . '\',\'View File\',1000)"><img src="'.base_url('/assets/icon/adadoc.png').'" style="width:20px"></a>';
				}else{
	...
	function index()
	{
		...
		echo $this->data['content'] = $this->load->view('workshop/index', $this->data, true);
	...
	function show($id = null)
	{
		...
		if ($row) {
			...
			$this->data['row'] = $this->model->getColumnTable('kursus_khusus');
		}

		$this->data['id'] = $id;
		echo $this->data['content'] =  $this->load->view('workshop/view', $this->data, true);
		//$this->load->view('layouts/main',$this->data);
	}
	...
	function add($id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['PEGAWAI_ID'] = $_POST['id'];
		echo $this->data['content'] = $this->load->view('workshop/form', $this->data, true);
		//$this->load->view('layouts/main', $this->data );

	}
	
	function viewfile($col,$id){
		$th = $this->db->query("SELECT $col FROM kursus_khusus WHERE KURSUS_KHUSUS_ID = '$id'")->row();
		$ext = explode(".",$th->$col);
		$maxext = count($ext);
		$extn = $ext[$maxext-1];
		if($extn == 'pdf'){
	...
	function save()
	{
		...
		*/
		...
		$_FILES["FILE_PDF"]["name"] = 'WORKSHOP_'.$nip.'_'.$tmt.'.pdf';
		if(!$this->upload->do_upload('FILE_PDF'))
		{
				$e = $this->upload->display_errors();
				$a = $e;
		}else{
			$namafile_sk = 'dokumen/'.$nip.'/WORKSHOP_'.$nip.'_'.$tmt.'.pdf';
		}
		}
		...
		if ($this->form_validation->run()) {
			...
			$datalama = $this->getdatalama($_POST, 'kursus_khusus', 'KURSUS_KHUSUS_ID', $data['KURSUS_KHUSUS_ID']);
			$ID = $this->model->insertRow($data, $this->input->get_post('KURSUS_KHUSUS_ID', true));
			$this->perubahandata($data['PEGAWAI_ID'], 'Workshop', 'kursus_khusus', json_encode($datalama), json_encode($_POST), 'KURSUS_KHUSUS_ID', $ID);
			// Input logs
			if ($this->input->get('KURSUS_KHUSUS_ID', true) == '') {
	...
	function destroy()
	{
		...
		$datalama = $this->getdatalamadelete('kursus_khusus', 'KURSUS_KHUSUS_ID', $_POST['id']);
		$this->perubahandata($datalama['PEGAWAI_ID'], 'Workshop', 'kursus_khusus', json_encode($datalama), 'DELETE', 'KURSUS_KHUSUS_ID', $_POST['id']);
