...
class Penilaiandptiga extends SB_Controller
{

	protected $layout 	= "layouts/main";
	public $module 		= 'penilaiandptiga';
	public $per_page	= '10';
	public $idx			= '';
	...
	function __construct()
	{
		...
		$this->load->model('penilaiandptigamodel');
		$this->model = $this->penilaiandptigamodel;
	...
	function grids($pg)
	{
		...
		foreach ($rows as $dt) {
			...
			if ($this->access['is_remove'] == 1) {
				$btn .= '<a class="dropdown-item waves-effect waves-light" href="#" onclick="ConfirmDelete(\'' . site_url('penilaiandptiga/destroy/') . '\',' . $dt->$idku . ')"><i class="ti-trash"></i> Delete</a>';
			}
			$btn .= '</div>';
			if($dt->FILE_PDF != ''){
				$row[] = '<a href="javascript:SximoModal(\'' . site_url('penilaiandptiga/viewfile/FILE_PDF') . '/' . $dt->$idku . '\',\'View File\',1000)"><img src="'.base_url('/assets/icon/adadoc.png').'" style="width:20px"></a>';
				}else{
	...
	function index()
	{
		...
		echo $this->data['content'] = $this->load->view('penilaiandptiga/index', $this->data, true);
	...
	function show($id = null)
	{
		...
		if ($row) {
			...
			$this->data['row'] = $this->model->getColumnTable('penilaian');
		}

		$this->data['id'] = $id;
		echo $this->data['content'] =  $this->load->view('penilaiandptiga/view', $this->data, true);
		//$this->load->view('layouts/main',$this->data);
	}
	...
	function add($id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['PEGAWAI_ID'] = $_POST['id'];
		echo $this->data['content'] = $this->load->view('penilaiandptiga/form', $this->data, true);
		//$this->load->view('layouts/main', $this->data );

	}
	
	function viewfile($col,$id){
		$th = $this->db->query("SELECT $col FROM penilaian WHERE PENILAIAN_ID = '$id'")->row();
		$ext = explode(".",$th->$col);
		$maxext = count($ext);
		$extn = $ext[$maxext-1];
		if($extn == 'pdf'){
	...
	function save()
	{
		...
		*/
		...
		$_FILES["FILE_PDF"]["name"] = 'DP3_'.$nip.'_'.$tmt.'.pdf';
		if(!$this->upload->do_upload('FILE_PDF'))
		{
				$e = $this->upload->display_errors();
				$a = $e;
		}else{
			$namafile_sk = 'dokumen/'.$nip.'/DP3_'.$nip.'_'.$tmt.'.pdf';
		}
		}
		...
		if ($this->form_validation->run()) {
			...
			$datalama = $this->getdatalama($_POST, 'penilaian', 'PENILAIAN_ID', $data['PENILAIAN_ID']);
			$ID = $this->model->insertRow($data, $this->input->get_post('PENILAIAN_ID', true));
			$this->perubahandata($data['PEGAWAI_ID'], 'penilaiandptiga', 'penilaian', json_encode($datalama), json_encode($_POST), 'PENILAIAN_ID', $ID);
			// Input logs
			if ($this->input->get('PENILAIAN_ID', true) == '') {
