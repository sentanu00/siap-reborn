...
class Tim_kerja extends SB_Controller
{

	protected $layout 	= "layouts/main";
	public $module 		= 'tim_kerja';
	public $per_page	= '10';
	public $idx			= '';
	...
	function __construct()
	{
		...
		$this->load->model('tim_kerjamodel');
		$this->model = $this->tim_kerjamodel;
...
<div class="dropdown-menu" x-placement="bottom-start" style="position: absolute; transform: translate3d(86px, 40px, 0px); top: 0px; left: 0px; will-change: transform;">';
			...
			if ($this->access['is_remove'] == 1) {
				$btn .= '<a class="dropdown-item waves-effect waves-light" href="#" onclick="ConfirmDelete(\'' . site_url('tim_kerja/destroy/') . '\',' . $dt->$idku . ')"><i class="ti-trash"></i> Delete</a>';
			}
			$btn .= '</div>';
			//tambahkan di bawah ini untuk preview file pdf
			if ($dt->FILE_PDF != '') { //--------------------------------rubah di bawah ini agar bisa preview file pdf
				$row[] = '<a href="javascript:SximoModal(\'' . site_url('tim_kerja/viewfile/FILE_PDF') . '/' . $dt->$idku . '\',\'View File\',1000)"><img src="' . base_url('/assets/icon/adadoc.png') . '" style="width:20px"></a>';
			} else {
	...
	function index()
	{
		...
		echo $this->data['content'] = $this->load->view('tim_kerja/index', $this->data, true);
	...
	function show($id = null)
	{
		...
		if ($row) {
			...
			$this->data['row'] = $this->model->getColumnTable('tim_kerja');
		}

		$this->data['id'] = $id;
		echo $this->data['content'] =  $this->load->view('tim_kerja/view', $this->data, true);
		//$this->load->view('layouts/main',$this->data);
	}
	...
	function add($id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['PEGAWAI_ID'] = $_POST['id'];
		echo $this->data['content'] = $this->load->view('tim_kerja/form', $this->data, true);
		//$this->load->view('layouts/main', $this->data );
	...
	{ //----------------------------------------GANTI NAMA TABEL DAN ID DI BAWAH INI ---------------------------------------
		$th = $this->db->query("SELECT $col FROM tim_kerja WHERE id = '$id'")->row();
		$ext = explode(".", $th->$col);
		$maxext = count($ext);
		$extn = $ext[$maxext - 1];
		if ($extn == 'pdf') {
	...
	function save()
	{
		...
		if ($tempfile_sk == '') {
			...
			$_FILES["FILE_PDF"]["name"] = 'TIM_KERJA_' . $nip . '_' . $tmt . '.pdf';
			if (!$this->upload->do_upload('FILE_PDF')) {
				$e = $this->upload->display_errors();
				$a = $e;
			} else {
				$namafile_sk = 'dokumen/' . $nip . '/TIM_KERJA_' . $nip . '_' . $tmt . '.pdf';
			}
		}
