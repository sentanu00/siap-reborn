...
class Penghargaan extends SB_Controller
{
    protected $layout = "layouts/main";
    public $module = "penghargaan";
    public $per_page = "10";
    public $idx = "";
    ...
    function __construct()
    {
        ...
        $this->model = $this->penghargaanmodel;
        ...
        foreach ($rows as $dt) {
            ...
            if ($dt->FILE_PDF != "") {
                $row[] =
                    ...
                    site_url("penghargaan/viewfile/FILE_PDF") .
                    "/" .
                    $dt->$idku .
    ...
    function show($id = null)
    {
        ...
        if ($row) {
            ...
            $this->data["row"] = $this->model->getColumnTable("penghargaan");
        }
    ...
    {
        $th = $this->db
            ->query("SELECT $col FROM penghargaan WHERE PENGHARGAAN_ID = '$id'")
            ->row();
        $ext = explode(".", $th->$col);
        $maxext = count($ext);
        $extn = $ext[$maxext - 1];
    ...
    //             $data,
    //             $this->input->get_post("PENGHARGAAN_ID", true)
    //         );
    //         $this->perubahandata(
    //             $data["PEGAWAI_ID"],
    //             "Penghargaan",
    //             "penghargaan",
    //             json_encode($datalama),
    //             json_encode($_POST),
    //             "PENGHARGAAN_ID",
    //             $ID
    //         );
    //         // Input logs
    //         if ($this->input->get("PENGHARGAAN_ID", true) == "") {
    //             $this->inputLogs(
    ...
    function save()
    {
        ...
        $namafile_sk = '';
        $tempfile_sk = $_FILES['FILE_PDF']['tmp_name'];
        $tmt = str_replace([' ', ':'], '_', $_POST['TANGGAL_SK']);
        ...
        if ($tempfile_sk == '') {
            ...
            $_FILES["FILE_PDF"]["name"] = 'PENGHARGAAN_' . $nip . '_' . $tmt . '.pdf';

            if (!$this->upload->do_upload('FILE_PDF')) {
                $e = $this->upload->display_errors();
                $a = $e;
            } else {
                $namafile_sk = 'dokumen/' . $nip . '/PENGHARGAAN_' . $nip . '_' . $tmt . '.pdf';
            }
        }
        ...
        if ($this->form_validation->run()) {
            ...
            // $datalama = $this->getdatalama($_POST, 'penghargaan', 'PENGHARGAAN_ID', $data['PENGHARGAAN_ID']);
            $ID = $this->model->insertRow($data, $this->input->get_post('PENGHARGAAN_ID', true));
            // $this->perubahandata($data['PEGAWAI_ID'], 'Penghargaan', 'penghargaan', json_encode($datalama), json_encode($_POST), 'PENGHARGAAN_ID', $ID);
            // Input logs
            if ($this->input->get('PENGHARGAAN_ID', true) == '') {
