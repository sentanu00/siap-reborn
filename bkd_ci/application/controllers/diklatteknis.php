...
class Diklatteknis extends SB_Controller
{

	protected $layout 	= "layouts/main";
	public $module 		= 'diklatteknis';
	public $per_page	= '10';
	public $idx			= '';
	...
	function __construct()
	{
		...
		$this->load->model('diklatteknismodel');
		$this->model = $this->diklatteknismodel;
	...
	where t.group_id = 1 and p.VALIDASI = 0');
		...
		foreach ($rows as $dt) {
			...
			if ($this->access['is_remove'] == 1) {
				$btn .= '<a class="dropdown-item waves-effect waves-light" href="#" onclick="ConfirmDelete(\'' . site_url('diklatteknis/destroy/') . '\',' . $dt->$idku . ')"><i class="ti-trash"></i> Delete</a>';
			}
			$btn .= '</div>';
			if ($dt->FILE_PDF != '') {
				$row[] = '<a href="javascript:SximoModal(\'' . site_url('diklatteknis/viewfile/FILE_PDF') . '/' . $dt->$idku . '\',\'View File\',1000)"><img src="' . base_url('/assets/icon/adadoc.png') . '" style="width:20px"></a>';
			} else {
				$row[] = '<img src="' . base_url('/assets/icon/nodoc.png') . '" style="width:20px">';
			}
			//$row[] = '<a href="javascript:SximoModal(\'' . site_url('dfsview/diklatteknis') . '/' . $pg . '/' . $dt->$idku . '\',\'Data DFS\',1000)">View File</a>';
			$row[] = $btn;
			$data[] = $row;
			$no++;
		}
	...
	function index()
	{
		...
		echo $this->data['content'] = $this->load->view('diklatteknis/index', $this->data, true);
	...
	function show($id = null)
	{
		...
		if ($row) {
			...
			$this->data['row'] = $this->model->getColumnTable('diklat_teknis');
		}

		$this->data['id'] = $id;
		echo $this->data['content'] =  $this->load->view('diklatteknis/view', $this->data, true);
		//$this->load->view('layouts/main',$this->data);
	}
	...
	function add($id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['PEGAWAI_ID'] = $_POST['id'];
		echo $this->data['content'] = $this->load->view('diklatteknis/form', $this->data, true);
		//$this->load->view('layouts/main', $this->data );
	...
	{
		$th = $this->db->query("SELECT $col FROM diklat_teknis WHERE DIKLAT_TEKNIS_ID = '$id'")->row();
		$ext = explode(".", $th->$col);
		$maxext = count($ext);
		$extn = $ext[$maxext - 1];
		if ($extn == 'pdf') {
	...
	function save()
	{
		...
		*/
		...
		if ($tempfile_sk == '') {
			...
			$_FILES["FILE_PDF"]["name"] = 'SK_DTEK_' . $nip . '_' . $tmt . '.pdf';
			if (!$this->upload->do_upload('FILE_PDF')) {
				$e = $this->upload->display_errors();
				$a = $e;
			} else {
				$namafile_sk = 'dokumen/' . $nip . '/SK_DTEK_' . $nip . '_' . $tmt . '.pdf';
			}
		}
		...
		if ($this->form_validation->run()) {
			...
			$datalama = $this->getdatalama($_POST, 'diklat_teknis', 'DIKLAT_TEKNIS_ID', $data['DIKLAT_TEKNIS_ID']);
			$ID = $this->model->insertRow($data, $this->input->get_post('DIKLAT_TEKNIS_ID', true));
			$this->perubahandata($data['PEGAWAI_ID'], 'diklatteknis', 'diklat_teknis', json_encode($datalama), json_encode($_POST), 'DIKLAT_TEKNIS_ID', $ID);
			// Input logs
			if ($this->input->get('DIKLAT_TEKNIS_ID', true) == '') {
	...
	function destroy()
	{
		...
		$datalama = $this->getdatalamadelete('diklat_teknis', 'DIKLAT_TEKNIS_ID', $_POST['id']);
		$this->perubahandata($datalama['PEGAWAI_ID'], 'Diklat Teknis', 'diklat_teknis', json_encode($datalama), 'DELETE', 'DIKLAT_TEKNIS_ID', $_POST['id']);
