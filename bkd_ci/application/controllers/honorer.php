...
class Honorer extends SB_Controller
{

	protected $layout 	= "layouts/main";
	public $module 		= 'honorer';
	public $per_page	= '10';
	public $idx			= '';
	...
	function grids()
	{
		...
		// if ($sttpeg != 'A')  $filter .= " AND STATUS_PEGAWAI IN ('11', '12', '13')";
		...
		$filter .= " AND (STATUS_PEGAWAI = '11' OR STATUS_PEGAWAI = '12' OR STATUS_PEGAWAI = '13'  OR STATUS_PEGAWAI = '14'  OR STATUS_PEGAWAI = '15'  OR STATUS_PEGAWAI = '16'  OR STATUS_PEGAWAI = '17' )";
...
<div class="dropdown-menu" x-placement="bottom-start" style="position: absolute; transform: translate3d(86px, 40px, 0px); top: 0px; left: 0px; will-change: transform;">';
			if ($this->access['is_edit'] == 1) {
				$btn .= '<a class="dropdown-item waves-effect waves-light" href=' . site_url('honorer/add/' . $dt->$idku) . '><i class="ti-pencil-alt"></i> Edit</a>';
			}
			if ($this->access['is_remove'] == 1) {
				// $btn .= '<a class="dropdown-item waves-effect waves-light" href="#" onclick="ConfirmDelete(\'' . site_url('honorer/destroy/') . '\',' . $dt->$idku . ')"><i class="ti-trash"></i> Delete</a>';
			}
			$btn .= '</div>';
	...
	function index()
	{
		...
		$this->data['content'] = $this->load->view('honorer/index', $this->data, true);
	...
	function show($id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['content'] =  $this->load->view('honorer/view', $this->data, true);
		$this->load->view('layouts/main', $this->data);
	}
	...
	function add($id = null)
	{
		...
		if ($row) {
			...
			// $this->data['fotoprofile'] =  $this->getfotoprofile($row['NIP_BARU']);
		...
		$this->data['id'] = $id;
		$this->data['content'] = $this->load->view('honorer/form', $this->data, true);
		$this->load->view('layouts/main', $this->data);
	}
	...
	function profile($id = null)
	{
		...
		$this->data['content'] = $this->load->view('honorer/profil', $this->data, true);
		$this->load->view('layouts/main', $this->data);
	}

	function satkerpilih()
	{
		echo $this->load->view('honorer/treesatker', $this->data, true);
	}
	...
	function identitas()
	{
		...
		$this->load->view('honorer/formpegawai', $this->data);
	}
	...
	function save()
	{
		...
		if ($this->form_validation->run()) {
			...
			$nameFOTO_SETENGAH = "foto_setengah_" . $_POST['NIP_BARU'] . ".jpeg";
	...
	function updatelastdatajabatan()
	{
		...
		$rx = $this->db->query("SELECT PEGAWAI_ID FROM pegawai WHERE STATUS_PEGAWAI IN (11,12,13)")->result();
		foreach ($rx as $k) {
