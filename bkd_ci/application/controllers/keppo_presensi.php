...
class Keppo_presensi extends SB_Controller
{

	protected $layout 	= "layouts/main";
	public $module 		= 'keppo_presensi';
	public $per_page	= '10';
	public $idx			= '';
	...
	function __construct()
	{
		...
		$this->load->model('keppo_presensimodel');
		$this->model = $this->keppo_presensimodel;
	...
	where t.group_id = 1 and p.VALIDASI = 0');
		...
		foreach ($rows as $dt) {
			...
			if ($this->access['is_remove'] == 1) {
				$btn .= '<a class="dropdown-item waves-effect waves-light" href="#" onclick="ConfirmDelete(\'' . site_url('keppo_presensi/destroy/') . '\',' . $dt->$idku . ')"><i class="ti-trash"></i> Delete</a>';
			}
			$btn .= '</div>';
			if ($dt->FILE_PDF != '') {
				$row[] = '<a href="javascript:SximoModal(\'' . site_url('keppo_presensi/viewfile/FILE_PDF') . '/' . $dt->$idku . '\',\'View File\',1000)"><img src="' . base_url('/assets/icon/adadoc.png') . '" style="width:20px"></a>';
			} else {
	...
	function index()
	{
		...
		echo $this->data['content'] = $this->load->view('keppo_presensi/index', $this->data, true);
	...
	function show($id = null)
	{
		...
		$this->data['id'] = $id;
		echo $this->data['content'] =  $this->load->view('keppo_presensi/view', $this->data, true);
		//$this->load->view('layouts/main',$this->data);
	}
	...
	function add($id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['PEGAWAI_ID'] = $_POST['id'];
		echo $this->data['content'] = $this->load->view('keppo_presensi/form', $this->data, true);
		//$this->load->view('layouts/main', $this->data );
	...
	function save()
	{
		...
		if ($this->form_validation->run()) {
			...
			$ID = $this->model->insertRow($data, $this->input->get_post('KEPPO_PRESENSI_ID', true));
			// Input logs
			...
			if ($this->input->post('apply')) {
				redirect('keppo_presensi/add/' . $ID, 301);
			} else {
				redirect('keppo_presensi', 301);
			}
		} else {
			$data =	array(
