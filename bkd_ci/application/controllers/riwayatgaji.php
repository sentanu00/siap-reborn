...
class Riwayatgaji extends SB_Controller
{

	protected $layout 	= "layouts/main";
	public $module 		= 'riwayatgaji';
	public $per_page	= '10';
	public $idx			= '';
	...
	function __construct()
	{
		...
		$this->load->model('riwayatgajimodel');
		$this->model = $this->riwayatgajimodel;
	...
	where t.group_id = 1 and p.VALIDASI = 0');
		...
		foreach ($rows as $dt) {
			...
			for ($i = 0; $i < count($this->col); $i++) {
				$field = $this->col[$i + 1];
				if ($field == 'MASA_KERJA_TAHUN') {
					$row[] = $dt->$field . ' Thn ' . $dt->MASA_KERJA_BULAN . ' bln';
				} else {
...
<div class="dropdown-menu" x-placement="bottom-start" style="position: absolute; transform: translate3d(86px, 40px, 0px); top: 0px; left: 0px; will-change: transform;">';
			...
			if ($this->access['is_remove'] == 1) {
				$btn .= '<a class="dropdown-item waves-effect waves-light" href="#" onclick="#"><i class="ti-eye"></i> View File</a>';
				$btn .= '<a class="dropdown-item waves-effect waves-light" href="#" onclick="#"><i class="ti-file"></i> Upload File</a>';
				$btn .= '<a class="dropdown-item waves-effect waves-light" href="#" onclick="ConfirmDelete(\'' . site_url('riwayatgaji/destroy/') . '\',' . $dt->$idku . ')"><i class="ti-trash"></i> Delete</a>';
			}
			$btn .= '</div>';
			if ($dt->FILE_PDF != '') {
				$row[] = '<a href="javascript:SximoModal(\'' . site_url('riwayatgaji/viewfile/FILE_PDF') . '/' . $dt->$idku . '\',\'View File\',1000)"><img src="' . base_url('/assets/icon/adadoc.png') . '" style="width:20px"></a>';
			} else {
				$row[] = '<img src="' . base_url('/assets/icon/nodoc.png') . '" style="width:20px">';
			}
			// $row[] = '<a href="javascript:SximoModal(\'' . site_url('dfsview/riwayatgaji') . '/' . $pg . '/' . $dt->$idku . '\',\'Data DFS\',1000)">View File</a>';
			$row[] = $btn;
			$data[] = $row;
			$no++;
		}
	...
	function index()
	{
		...
		echo $this->data['content'] = $this->load->view('riwayatgaji/index', $this->data, true);
	...
	function show($id = null)
	{
		...
		if ($row) {
			...
			$this->data['row'] = $this->model->getColumnTable('gaji_riwayat');
		}

		$this->data['id'] = $id;
		echo $this->data['content'] =  $this->load->view('riwayatgaji/view', $this->data, true);
		//$this->load->view('layouts/main',$this->data);
	}
	...
	function add($id = null)
	{
		...
		$this->data['id'] = $id;
		$this->data['PEGAWAI_ID'] = $_POST['id'];
		echo $this->data['content'] = $this->load->view('riwayatgaji/form', $this->data, true);
		//$this->load->view('layouts/main', $this->data );
	...
	{
		$th = $this->db->query("SELECT $col FROM gaji_riwayat WHERE GAJI_RIWAYAT_ID = '$id'")->row();
		$ext = explode(".", $th->$col);
		$maxext = count($ext);
		$extn = $ext[$maxext - 1];
		if ($extn == 'pdf') {
	...
	function save()
	{
		...
		*/
		...
		if ($tempfile_sk == '') {
			...
			$_FILES["FILE_PDF"]["name"] = 'KGB_' . $nip . '_' . $tmt . '_' . $tanggal_jam . '.pdf';
			if (!$this->upload->do_upload('FILE_PDF')) {
				$e = $this->upload->display_errors();
				$a = $e;
			} else {
				$namafile_sk = 'dokumen/' . $nip . '/KGB_' . $nip . '_' . $tmt . '_' . $tanggal_jam . '.pdf';
			}
		}
		...
		if ($this->form_validation->run()) {
			...
			$datalama = $this->getdatalama($_POST, 'gaji_riwayat', 'GAJI_RIWAYAT_ID', $data['GAJI_RIWAYAT_ID']);
			$ID = $this->model->insertRow($data, $this->input->get_post('GAJI_RIWAYAT_ID', true));
			$this->perubahandata($data['PEGAWAI_ID'], 'Riwayat Gaji', 'gaji_riwayat', json_encode($datalama), json_encode($_POST), 'GAJI_RIWAYAT_ID', $ID);

			$this->getlastriwayat('gaji_riwayat', $data['PEGAWAI_ID'], 'TMT_SK');
			// Input logs
			if ($this->input->get('GAJI_RIWAYAT_ID', true) == '') {
			...
			if ($this->input->post('apply')) {
				redirect('riwayatgaji/add/' . $ID, 301);
			} else {
				redirect('riwayatgaji', 301);
			}
		} else {
			$data =	array(
	...
	function destroy()
	{
		...
		$datalama = $this->getdatalamadelete('gaji_riwayat', 'GAJI_RIWAYAT_ID', $_POST['id']);
		$this->perubahandata($datalama['PEGAWAI_ID'], 'Riwayat Gaji', 'gaji_riwayat', json_encode($datalama), 'DELETE', 'GAJI_RIWAYAT_ID', $_POST['id']);
