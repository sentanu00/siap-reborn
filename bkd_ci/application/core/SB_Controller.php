...
class SB_Controller extends CI_Controller
{
		...
		foreach ($str as $f) {
			...
			if ($f['view'] == 1) {
				...
				switch ($f['type']) {
					...
					case 'checkbox':
						//var_dump($this->input->get_post( $field , true ));die();
						if (!$this->input->get_post($field, true)) {
							$data[$field] = 0;
						} else {
	...
	function perubahandata($pegawaiid, $fip, $tablex, $datalama, $databaru, $dbkey, $dbkeyval)
	{
		$user = $this->session->userdata('fid');

		$data = array(
			'DB_KEY'		=> $dbkey,
			'DB_KEY_VALUE'	=> $dbkeyval,
			'PEGAWAI_ID'	=> $pegawaiid,
			'FORM_FIP'		=> $fip,
			'DB_TABLE'		=> $tablex,
			'ISI_LAMA'		=> $datalama,
			'ISI_BARU'		=> $databaru,
			'USER_APP_ID'	=> $this->session->userdata('username'),
			'LAST_CREATE_USER' => $this->session->userdata('username'),
			'LAST_CREATE_DATE' => date('Y-m-d H:i:s')
		);
		$this->db->insert('perubahan_data', $data);
	}

	function getdatalamadelete($table, $fieldid, $valid)
	{

		$datalama = $this->db->query("SELECT * FROM " . $table . " WHERE " . $fieldid . " = '" . $valid . "'")->row_array();
		return $datalama;
	}

	function getdatalama($post, $table, $fieldid, $valid)
	{
		foreach ($post as $key => $value) {
			$fld[] = $key;
		}
		$field = implode(", ", $fld);
		$datalama = $this->db->query("SELECT " . $field . " FROM " . $table . " WHERE " . $fieldid . " = '" . $valid . "'")->row_array();
		return $datalama;
	}
	...
	function getStatusdfs($pegid, $id, $jenis)
	{
		$a = $this->db->query("SELECT nama_file from dokumen where fid_pegawai = '$pegid' and id = '$id' and fid_jenis_dokumen = '$jenis'")->row();
		$d = 0;
		if ($a) {
			$d = 1;
		}
	...
	function getlastriwayat($table, $pegid, $col)
	{
		$this->db->query("UPDATE $table SET FLAG_DATA_TERAKHIR = 0 WHERE PEGAWAI_ID = '$pegid'");
		$a = $this->db->query("SELECT MAX($col) as maxim FROM $table WHERE PEGAWAI_ID = '$pegid'")->row();
		$datamax = $a->maxim;
		$this->db->query("UPDATE $table SET FLAG_DATA_TERAKHIR = 1 WHERE PEGAWAI_ID = '$pegid' AND $col = '$datamax'");
	}
}
