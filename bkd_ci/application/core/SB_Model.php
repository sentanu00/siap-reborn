...
class SB_Model extends CI_Model
{
	...
	public function getRowsCustom($args)
	{
		$table = $this->table;
		$key = $this->primaryKey;
		...
		if ($limit > 0) {
			...
			$limitConditional = ""; // hilangkan limit kalau 0
		}
		$orderConditional = ($sort != '' && $order != '') ?  " ORDER BY {$sort} {$order} " : '';
		...
		$sql = $this->querySelect() . $this->queryWhere();
		...
		$sql .= $this->queryGroup() . " {$orderConditional} {$limitConditional} ";

		// --- Debug dulu kalau mau lihat query final:
	...
	function getComboselect($params, $nested = array())
	{
		if (isset($params[3]) and !empty($params[4])) {
			$table = $params[0];
			$query = $this->db->get_where($table, array($params[3] => $params[4]), 10000, 0);
			if (isset($params[5]) and !empty($params[6])) {
				$query = $this->db->get_where($table, array($params[3] => $params[4], $params[5] => $params[6]), 10000, 0);
			}
			if (isset($params[7]) and !empty($params[8])) {
				$query = $this->db->get_where($table, array($params[7] => $params[8], $params[3] => $params[4], $params[5] => $params[6]), 10000, 0);
			}
			$rows = $query->result();
			$query->free_result();
			return $rows;
