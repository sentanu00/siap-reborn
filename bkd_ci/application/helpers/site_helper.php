...
class SiteHelpers
{
	...
	public static function getStatusUsulan($b)
	{
		$a = array(
			'0'	=> 'Draft',
			'1'	=> 'Kirim Data ke BKSDM'
		);
		return $a[$b];
	}

	public static function getStatusUsulanPegawai($b)
	{
		$a = array(
			'0'	=> '<span class="badge badge-secondary" style="background-color:black">Input Berkas',
			'1'	=> '<span class="badge badge-primary" style="background-color:black">Selesai Input Berkas',
			'2'	=> '<span class="badge badge-info" style="background-color:blue">Kirim BKSDM',
			'3'	=> '<span class="badge badge-info" style="background-color:black">Validasi',
			'4'	=> '<span class="badge badge-info" style="background-color:yellow">BKN Pusat',
			'5'	=> '<span class="badge badge-success" style="background-color:green">SK Terbit',
			'6'	=> '<span class="badge badge-danger" style="background-color:red">Ditolak</span>',
		);
		return $a[$b];
	}
	public static function getdokumensyarat($idpegawai, $idusulan, $idsyarat, $table_file = '', $col_file = '', $dbjenisdok = '', $singlefile = '')
	{
		$_this = &get_Instance();
		$cekfile1 = $_this->db->query("SELECT * FROM ep_tx_usulan_pemberhentian_syarat WHERE pegawai_id='$idpegawai' AND id_usulan_detail='$idusulan' AND id_syarat='$idsyarat'")->row();
		if ($cekfile1) {
			return $cekfile1->file_syarat;
		} else {
			if ($dbjenisdok != '' || $dbjenisdok != 0) {
				$cekfile2 = $_this->db->query("SELECT nama_file FROM dokumen WHERE fid_pegawai='$idpegawai' AND fid_jenis_dokumen='$dbjenisdok'")->row();
				if ($cekfile2) {

					return $cekfile2->nama_file;
				}
			}

			if ($table_file != '' && $col_file != '') {
				if ($singlefile == 1) {
					$cekfile2 = $_this->db->query("SELECT $col_file FROM $table_file WHERE PEGAWAI_ID='$idpegawai'")->row();
					if ($cekfile2) {
						return $cekfile2->$col_file;
					}
				} else if ($singlefile == 2) {
					$cekfile3 = $_this->db->query("SELECT $col_file FROM $table_file WHERE PEGAWAI_ID='$idpegawai' AND FLAG_DATA_TERAKHIR=1")->row();
					if ($cekfile3) {
						return $cekfile3->$col_file;
					}
				} else {
					//belum yang all file per pegawai
				}
			}
		}
	}
	...
	public static function getReporting($tgl)
	{
	...
	public static function showUploadedFile($file, $path, $width = 50)
	{
		...
		if (file_exists('./' . $files) && $file != "") {
			...
			if (isset($info['extension'])) {
				if (
					$info['extension'] == "jpg" || $info['extension'] == "jpeg" ||  $info['extension'] == "png"
					|| $info['extension'] == "gif"
				) {
					$path_file = str_replace("./", "", $path);
	...
	public static function formShow($type, $field, $required, $option = array())
	{
		...
		if ($required == 'required') {
			...
			$mandatory = "required parsley-type='date' ";
		...
		switch ($type) {
			...
			case 'text_date';
				$form = "
				<input type='date' class='form-control input-sm' placeholder='' value='<?php echo \$row['{$field}'];?>' name='{$field}'
